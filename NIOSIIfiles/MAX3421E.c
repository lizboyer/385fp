...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int returncode;
	alt_u8* write_data = &reg;
	alt_u8 selector[2];
	selector[0] = reg + 2;
	selector[1] = val;
	returncode = alt_avalon_spi_command(SPI_0_BASE, 0,2,selector, 0,NULL,0);
	if (returncode < 0) printf ("MAXreg_wr Error");
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	returncode = alt_avalon_spi_command(SPI_0_BASE, 0, nbytes + 1, arr, 0, NULL, 0);

	if (returncode < 0) printf ("MAXbytes_wr Error");
...
BYTE MAXreg_rd(BYTE reg) {
	...
	selector[0] = reg + 2;
	// selector[1] = val;
	returncode = alt_avalon_spi_command(SPI_0_BASE, 0,1, &reg /*length of reg, 8 or 1 or write_data*/, 1, &read_data,0);
	if (returncode < 0) printf ("MAXreg_rd Error");
	return read_data;
}
//multiple-byte write
//returns a pointer to a memory position after last written
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//return (data + nbytes);
	int returncode;
	alt_u8 selector[nbytes];

	returncode = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, nbytes, data, 0);
